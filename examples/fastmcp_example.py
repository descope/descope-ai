...
def example_google_calendar_tool():
    ...
    async def get_calendar_events_simple(
        ...
        try:
            ...
            # Step 3: Call Google Calendar API with connection token
            if not httpx:
...
def example_connection_token_with_management_key():
    """Example: Get connection token using different authentication methods."""
    ...
    token1 = get_connection_token(
        user_id=user_id,
        app_id=app_id,
        scopes=["https://www.googleapis.com/auth/calendar.readonly"]
    )
    
    project_id = os.getenv("DESCOPE_PROJECT_ID", "P123456")
    management_key = os.getenv("DESCOPE_MANAGEMENT_KEY", "")
    
    token2 = get_connection_token(
        user_id=user_id,
        app_id=app_id,
        scopes=["https://www.googleapis.com/auth/calendar.readonly"],
...
def main():
    ...
    example_connection_token_with_management_key()
